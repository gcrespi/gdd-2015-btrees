...
AS
    BEGIN
	   ...
	   SELECT V.ViajeID,
			  ...
			  W.ButacasDisponibles AS 'Cant Disponibles'
...
/***** GetCantKgDisponiblesViaje *****/
IF  object_id(N'[THE_BTREES].[GetCantKgDisponiblesViaje]','P') IS NOT NULL
	DROP PROCEDURE [THE_BTREES].[GetCantKgDisponiblesViaje]
GO

CREATE PROCEDURE [THE_BTREES].[GetCantKgDisponiblesViaje]		
    @Viaje AS INT
...
AS
    BEGIN
	   ...
	   SELECT A.Avion_CantidadKgsDisponibles AS KSTotales,
			 (A.Avion_CantidadKgsDisponibles-(SUM(E.Enco_KG))) AS KGDisponibles
	   FROM THE_BTREES.Avion A
	   INNER JOIN THE_BTREES.Viaje V ON V.ViajeID=@Viaje AND A.AvionID=V.Viaje_AvionRef
	   INNER JOIN THE_BTREES.Encomienda E ON E.Enco_ViajeRef=V.ViajeID
	   GROUP BY E.Enco_ViajeRef,A.Avion_CantidadKgsDisponibles
